...
function createGoogleCalendarEvent($client_nom, $client_email, $client_telephone, $service_nom, $date, $heure, $service_duree)
{
    ...
    // Log pour débogage
    $log_message = "=== WEBHOOK CONFIRMATION " . date('Y-m-d H:i:s') . " ===\n";
    $log_message .= "Client: $client_nom\n";
    $log_message .= "Service: $service_nom\n";
    $log_message .= "Date: $date\n";
    $log_message .= "Heure: $heure\n";
    $log_message .= "Status HTTP: $status\n";
    if ($error) {
        $log_message .= "Erreur cURL: $error\n";
    }
    $log_message .= "Réponse: $result\n";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $service_id = isset($_POST['service']) ? ($_POST['service'] == 'indien' ? 1 : 2) : 0; // 1 pour Indien, 2 pour Tanin
